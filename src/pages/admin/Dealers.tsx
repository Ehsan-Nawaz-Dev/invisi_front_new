    ...
    <div style={{ display: 'grid', gap: 12 }}>
      <style>{`
        ...
        @media (max-width: 480px) {
          ...
          .modal-card {
            width: min(640px, 98vw) !important;
      ...
      <div className="dealers-table-wrap">
        <div className="dealers-table-header">
        ...
        <div style={{ display: 'grid' }}>
          {loading ? (
            Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="dealers-table-row" style={{ opacity: 0.7 }}>
            ...
            rows.map((r, idx) => (
              <button key={r.key} onClick={() => open(r.raw)} className="row-anim dealers-table-row" style={{ textAlign: 'left', background: 'transparent', color: 'inherit', cursor: 'pointer', animationDelay: `${idx * 30}ms` }}>
        ...
        <div role="dialog" aria-modal onClick={() => setInviteOpen(false)} className="modal-backdrop" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'grid', placeItems: 'center', zIndex: 50 }}>
          <div onClick={(e) => e.stopPropagation()} className="modal-card" style={{ width: 'min(640px, 92vw)', background: '#0B121B', border: '1px solid #223', borderRadius: 14, padding: 16, boxShadow: '0 20px 60px rgba(0,0,0,0.45)' }}>
            ...
            <form onSubmit={async (e) => {
              ...
              <div className="form-grid-2col" style={{ display: 'grid', gap: 10, gridTemplateColumns: '1fr 1fr' }}>
